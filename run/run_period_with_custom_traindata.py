"""
run_period_with_custom_traindata.py

功能说明：
此脚本实现了一个自定义训练数据生成的周期实验。与随机分割训练数据不同，
本脚本根据指定数量等距生成训练数据点，并且确保包含数据范围的第一个和最后一个点。
实验通过改变sin函数的周期参数，研究Softplus激活函数在不同周期下的拟合效果。

主要功能：
1. 等距生成训练数据点，确保包含区间端点
2. 固定网络宽度和beta参数，仅改变周期参数
3. 支持多神经元数量和多随机种子实验
4. 按周期组织实验结果并保存
"""
import os
...
# 3. 修改数据生成函数
# ==============================================================================
def generate_data_by_period(period, data_range, num_train_points, num_test_points):
    """
    生成具有自定义等距训练数据点的sin波数据集
    
    Args:
        period (float): 周期参数
        data_range (list): 数据范围，例如 [-2*pi, 2*pi]
        num_points (int): 总数据点数量
        num_train_points (int): 训练数据点数量
    ...
    # 生成训练集数据点
    X_train = np.linspace(data_range[0], data_range[1], num_train_points).reshape(-1, 1)
    y_train = np.sin(k * X_train)  # 使用计算出的频率生成sin波
    
    # 生成测试集数据点
    X_test = np.random.uniform(data_range[0], data_range[1], num_test_points).reshape(-1, 1)
    y_test = np.sin(k * X_test)  # 使用计算出的频率生成sin波
...
if __name__ == "__main__":
    ...
    for current_period in PERIOD_LIST:
        ...
        DATA_RANGE = [- current_period, current_period]
        # 1. 使用基于period的数据生成函数，并传入自定义训练点数量
        X_train, y_train, X_test, y_test = generate_data_by_period(
            current_period, DATA_RANGE, NUM_TRAIN_POINTS, NUM_TEST_POINTS
        )
