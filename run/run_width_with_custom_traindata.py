...
def generate_data(data_range, train_num_points, test_num_points):
    """
    生成并分割 sin(x) 数据集
    Args:
        data_range (list): 数据范围，例如 [-2*pi, 2*pi]
        train_num_points (int): 训练集数据点数量
        test_num_points (int): 测试集数据点数量
    Returns:
        X_train_t, y_train_t, X_test_t, y_test_t: 训练集和测试集的 PyTorch Tensors
    """
    X_train = np.linspace(data_range[0], data_range[1], train_num_points).reshape(-1, 1)
    y_train = np.sin(X_train)
    
    X_test = np.linspace(data_range[0], data_range[1], test_num_points).reshape(-1, 1)
...
def main():
    ...
    # 生成数据
    X_train, y_train, X_test, y_test = generate_data(DATA_RANGE, TRAIN_NUM_POINTS, TEST_NUM_POINTS)
