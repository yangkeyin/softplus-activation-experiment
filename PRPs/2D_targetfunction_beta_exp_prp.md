...
4. **指标对齐**：复现 run\_beta.py 中的核心时域指标（train\_rms, test\_rms）。  
...
5. **可视化修改**：  
   ...
   * 将 run\_beta.py 中的 beta\_rms\_seed.png (3x1 布局) 修改为 (2x1 布局)，仅保留 train\_rms 和 test\_rms 的演化图。
...
def generate\_data\_2D(data\_range, train\_points\_per\_dim, test\_points\_per\_dim, device):  
    ...
    """  
    \# 1\. 训练数据 (9x9 网格)  
    x\_ticks \= np.linspace(data\_range\[0\], data\_range\[1\], train\_points\_per\_dim)  
    y\_ticks \= np.linspace(data\_range\[0\], data\_range\[1\], train\_points\_per\_dim)  
    xx\_train, yy\_train \= np.meshgrid(x\_ticks, y\_ticks)  
    ...
    xx\_test, yy\_test \= np.meshgrid(x\_ticks\_test, y\_ticks\_test)  
    ...
    return X\_train, y\_train, X\_test, y\_test, xx\_train, yy\_train, xx\_test, yy\_test
...
* **数据**：调用 generate\_data\_2D(DATA\_RANGE, 9, 50, DEVICE)。  
...
* **指标计算**：在训练循环的评估步骤中，**只计算** train\_rms 和 test\_rms。移除 spectrum\_error。  
  \# results\[seed\]\[epoch \+ 1\]\["spectrum\_error"\] \= spectrum\_error \# \<--- 移除  
  \# rms\_data\[beta\]\[seed\]\[epoch \+ 1\]\["spectrum\_error"\] \= spectrum\_error \# \<--- 移除
...
* **目的**: **量化** beta 对学习速度和最终收敛精度的影响。

## **5\. 预期产物 (Deliverables)**

1. **Python 脚本**: new\_scripts/run\_beta\_2D.py (根据本 PRP 规划创建)。  
2. **实验结果**:  
   * figures/beta\_2D\_experiment/results\_base\_2D.pkl (包含 train\_rms 和 test\_rms 的结果字典)。  
3. **可视化图表**:  
   ...
   * ... (其他 beta 和 epoch)  
   * figures/beta\_2D\_experiment/beta\_rms\_seed.png (2x1 演化图)。