...
6. **保存结果**：将 finetune\_results 字典保存为 results\_finetune.pkl。
...
6. **global\_damage\_rms**: (新诊断指标) RMSE(y\_pred\_test\_ft, y\_test)（**只在**“远场” x\_far 区域计算）。衡量“原始知识”被破坏的程度。
...
将为**每个 beta**、**每个 scenario\_name** 和 **最终 epoch** 生成一个独立的 .png 文件。此图是一个 2x2 网格：

1. **子图1: 拟合函数对比图**  
   * **标题**: Fitted Function (Train: {N\_train}+{N\_outlier}, Test: {N\_test}) (**采纳建议2**)  
   * **线 (Mean)**: y\_test (黑), y\_pred\_base (绿), y\_pred\_finetuned (红)  
   * **范围 (Fill)**: y\_pred\_finetuned (浅红)  
   * **散点 (Scatter)**:  
     * x\_test vs y\_test (灰色, 小点) (**采纳建议1**)  
     * x\_train\_ft\_scatter vs y\_train\_ft\_scatter (蓝色, 大点) (原始训练点+异常点)  
2. **子图2: 微调误差图 (Finetuned Error)**  
   * **标题**: Finetuned Error (Avg. test\_rms\_ft: {value:.4f}) (**采纳建议3**)  
   * **线 (Mean)**: y\_pred\_finetuned \- y\_test (红)  
   * **范围 (Fill)**: (浅红)  
   * **散点 (Scatter)**: x\_test vs y\_pred\_finetuned \- y\_test (红色, 小点) (**采纳建议1**)  
   * **洞察**: *微调后的模型在哪些地方错了？*  
3. **子图3: 微调影响图 (Finetuning Impact)**  
   * **标题**: Finetuning Impact (Avg. y\_distortion\_rms: {value:.4f}) (**采纳建议3**)  
   * **线 (Mean)**: y\_pred\_finetuned \- y\_pred\_base (蓝)  
   * **范围 (Fill)**: (浅蓝)  
   * **散点 (Scatter)**: x\_test vs y\_pred\_finetuned \- y\_pred\_base (蓝色, 小点) (**采纳建议1**)  
   * **洞察**: *微调这个动作本身造成了多大的形状改变？*  
4. **子图4: 频谱对比 (Spectrum)**  
   * **标题**: Spectrum Comparison (Avg. spectrum\_error\_ft: {value:.4f}) (**采纳建议3**)  
   * **线 (Mean)**: true\_coef (黑), coef\_base (绿), coef\_finetuned (红) (均为log尺度)  
   * **散点 (Scatter)**: k vs coef\_... (用小点标出) (**采纳建议1**)

### **B. 图表类型2：全局汇总对比 (Summary Plots)**

将生成汇总所有 beta 的 .png 文件。

1. **图 summary\_metrics\_vs\_finetune\_epoch.png** (按Epoch演化)  
   * **X**: finetune\_epoch (从 1 到 MAX\_FINETUNE\_EPOCHS 的连续轴)  
   * **布局**: 2x2 网格，每个子图代表一个关键指标：  
     * 子图1: test\_rms\_ft  
     * 子图2: spectrum\_error\_ft  
     * 子图3: local\_fit\_error  
     * 子图4: global\_damage\_rms  
   * **线条**: 每个子图包含**多条线**（带颜色范围），每条线代表一个 beta 值。  
   * **说明**: 此图表将为**每个 scenario\_name 配置**单独生成。  
   * **洞察**: *模型是如何（以及多快）被破坏/扭曲的？*  
2. **图 summary\_tradeoff\_vs\_beta.png** (最终权衡)  
   * **X**: beta (log 尺度)  
   * **Y (左轴, 红色)**: global\_damage\_rms (在 MAX\_FINETUNE\_EPOCHS 时)。  
   * **Y (右轴, 蓝色)**: local\_fit\_error (在 MAX\_FINETUNE\_EPOCHS 时)。  
   * **线条**: 此图将同时绘制**多条线**（带颜色范围），例如：  
     * 'Scenario\_1\_Single' (实线)  
     * 'Scenario\_2\_Multiple' (虚线)  
   * **洞察**: *这*是回答您核心问题的最终图表。它将展示高Beta模型如何以“高局部误差”为代价换取“低全局损伤”；而低Beta模型则相反。并且能对比不同“攻击情景”带来的影响。