...
def train_and_analyze(
    ...
    # --- 1. 数据准备 ---
    x_data, y_data = create_sliding_windows(data, SEQ_LEN, PRED_LEN)
    
    # 找到测试样本 (用于绘图)
    x_test_sample = x_data[-1]
    y_test_sample = y_data[-1]
    
    # 根据模式对数据进行预处理
    if preprocess_mode == 'time_non_norm':
         # [cite: 189] (Fig 2b 左)
        x_train_tensor = torch.tensor(x_data, dtype=torch.float32).unsqueeze(-1).to(DEVICE)
        y_train_tensor = torch.tensor(y_data, dtype=torch.float32).unsqueeze(-1).to(DEVICE)
        ...
        y_fft = np.fft.fft(y_data, axis=1)
...
def run_figure_2a():
    ...
    im = ax_left_heatmap.imshow(errors_left.T, aspect='auto', cmap='gray', vmin=0, vmax=1.0)
    ...
    im = ax_right_heatmap.imshow(errors_right.T, aspect='auto', cmap='gray', vmin=0, vmax=1.0)
