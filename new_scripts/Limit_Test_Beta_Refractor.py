...
def parse_args():
    parser = argparse.ArgumentParser(description="分析 N < Width 时，初始化 Std 的影响")
    
    # 1. 核心实验变量 (我们要研究的对象)
    parser.add_argument('--train_points', type=int, default=10, help='训练点数 (建议设为 < width)')
    parser.add_argument('--init_std', type=float, default=None, help='权重初始化的标准差')
    
    # 2. 也是变量，但这次实验可能固定住
    parser.add_argument('--width', type=int, default=100, help='神经网络宽度')
    ...
    parser.add_argument('--degree', type=int, default=5, help='Legendre 多项式次数')
...
class MLP(nn.Module):
    ...
    def __init__(self, width, beta, init_std = None):
        super().__init__()
...
def main():
    ...
    # 4. 初始化模型 (传入 args)
    model = MLP(width=args.width, beta=args.beta, init_std=args.init_std).to(device)
    # 获取模型的std
    with torch.no_grad():
    ...
    plt.scatter(range(degree + 1), gt_coeffs, color='k', alpha=0.6, label='Ground Truth Spectrum')
    ...
    plt.title(f"Spectral Analysis: N={args.train_points}, Width={args.width}, std={init_std:.4f}")
    ...
    plt.plot(x_test_np, y_test_ground_truth, 'k-', alpha=0.2, label='Target')
    ...
    plt.title(f"Fit Test: MLP(Beta={args.beta}) vs Linear Interpolation")
    plt.legend()
    ...
    print(f"Final Result: Linear MSE={mse_linear:.6f}, MLP 0MSE={mse_mlp:.6f}")
